...
class HotfixCLI {
  ...
  async run(): Promise<void> {
    try {
      ...
      console.error('❌ Hotfix workflow failed:', error instanceof Error ? error.message : error);
      await this.rollback();
      process.exit(1);
    }
  }
  ...
  private async validateEnvironment(): Promise<void> {
    ...
    // Get current branch
    this.originalBranch = this.executeCommand('git branch --show-current').trim();
    
    // Check if we're on main branch
    if (this.originalBranch !== 'main') {
      throw new Error(`Must be on main branch. Currently on: ${this.originalBranch}`);
    }
  ...
  private async createPR(): Promise<void> {
    ...
    this.executeCommand(`gh pr create --title "${title}" --body "${body}" --base main --head ${this.branchName}`);
  ...
  private async mergePR(): Promise<void> {
    ...
    this.executeCommand(`gh pr merge ${this.branchName} --merge --delete-branch --auto`);
  ...
  private async rollback(): Promise<void> {
    ...
    try {
      ...
      console.error('⚠️ Rollback failed:', error instanceof Error ? error.message : error);
    }
  }
